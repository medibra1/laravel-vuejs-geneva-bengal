...
function submit(): void {
    if (props.cat) {
        form.transform((data) => ({ ...data, _method: 'put' })).post(
            route('admin.cats.adoption.update', props.cat.id),
            { forceFormData: true },
        );
    } else {
        form.post(route('admin.cats.adoption.store'), { forceFormData: true });
    }
}
...
function deletePhoto(photoId: number): void {
    ...
    if (confirm('Supprimer cette photo ?')) {
        router.delete(route('admin.cats.adoption.photos.destroy', [props.cat.id, photoId]), { preserveScroll: true });
    }
}
</script>
...
<template>
    ...
    <Head :title="cat ? `Modifier ${cat.name}` : 'Nouveau chaton'" />

    <AdminLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-white">
                {{ cat ? `Modifier ${cat.name}` : 'Nouveau chaton' }}
            </h2>
        </template>
